...
class ThatLowCarbLifeScraper extends BaseScraper {
  ...
  scrape($) {
    ...
    inst.children('ol').children('li').each((i, el) => {
      instructions.push($(el).text());
    });

    time.prep = this.textTrim($(".mv-create-time-prep .mv-create-time-format"));
    time.cook = this.textTrim($(".mv-create-time-active .mv-create-time-format"));
    time.total = this.textTrim($(".mv-create-time-total .mv-create-time-format"));
    this.recipe.servings = this.textTrim($(".mv-create-time-yield .mv-create-time-format"));
